...
<div id="help-search-container-banner-id" class="media-photo media-photo-block help-search-container help-search-container-banner">
  <div class="media-cover background-cover help-search-bg" style="background-image:url({{$help_page_cover_image}})" ></div>
  <div class="va-container va-container-v va-container-h">
    <div class="va-middle text-center text-contrast">
      ...
      <div class="page-container-responsive">
        <div class="row">
          <form class="help-search-form col-md-10 col-md-offset-1">
...
<div class="page-container page-container-responsive">
  <div class="row row-space-6 space-top-lg-6 space-top-md-4">
    <div class="col-md-3 left-menu">
      <div class="sub_menu_header show-sm">
  <h2>help menu</h2>
  ...
  <li>
     @if($category[$i]->active_subcategory_count($category[$i]->category_id)->count())
       @php $category[$i]->active = 'yes'; @endphp 
      @endif
    <a href="{{ ((count($category[$i]->subcategory)) && ($category[$i]->active == 'yes')) ? 'javascript:void(0);' : url('help/topic/'.$category[$i]->category_id.'/'.str_slug($category[$i]->category_name,'-')) }}" class="sidenav-item {{ ($category[$i]->active == 'yes') ? 'navtree-next' : '' }}" data-id="{{ $category[$i]->category_id }}" data-name="{{ $category[$i]->category->name }}" aria-selected="{{ ((Route::current()->uri() == 'help/topic/{id}/{category}' || Route::current()->uri() == 'help/article/{id}/{question}') && ($category[$i]->category->id == @$result[0]->category_id)) ? 'true' : 'false' }}"> {{ $category[$i]->category->name }}
      <span class="show-sm"><i class="icon icon-chevron-right"></i></span>
    </a>
    @if($category[$i]->active == 'yes') 
...
<div class="col-sm-12 col-sm-offset-0 col-md-9 help-content text-copy navtree-content breadcrumbs-content">
      <div class="h3 space-md-2 space-lg-4 help-center-sm black_bold">
...
<div class="help-center-sm">
  <div class="space-8">
    <h2 class="black_bold">{{ str_replace('SITE_NAME', $site_name, $result[0]->question) }}</h2>
...
@else
      <div class="col-md-9 help-content text-copy navtree-content breadcrumbs-content">
      
<div class="popular-topics">
  <div class="row-space-6">
    <div class="h2 space-md-2 space-lg-2 help-section-header">
      <h2 class="black_bold">{{ trans('messages.help.suggested_helps') }}</h2>
    </div>
